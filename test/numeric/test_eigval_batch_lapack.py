...
"""Tests for EigvalBatchSolver and CardanoSolver (cubic interface)."""
...
@pytest.fixture(params=["cardano", "eigval_batch"])
def solver(request):
...
class TestSolversAgree:
    """Cross-check that both implementations agree on the same inputs."""
    ...
    def test_random_polynomials_agree(self):
        """Both solvers must return the same max-real root for random cubics

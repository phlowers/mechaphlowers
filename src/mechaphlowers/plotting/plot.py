...
def get_support_points(data: pd.DataFrame) -> np.ndarray:
    ...
    # add nan to separate
    pp_final = np.concat([pp0.T, pp_up.T, np.nan*pp0.T, pp_up.T, pp_arm.T, np.nan*pp0.T,], axis = 1)
...
def get_insulator_points(data: pd.DataFrame) -> np.ndarray:
    ...
    # add nan to separate
    pp_final = np.concat([pp0.T, pp_insulator.T, np.nan*pp0.T,], axis = 1)
...
def set_layout(fig: go.Figure, auto: bool = True) -> None:
    ...
    zoom:float = .1 if auto else 1# perhaps this approx of the zoom will not be adequate for all cases
    aspect_ratio: Dict = dict( x=1, y=.05, z=.5 )    
...
class PlotAccessor:
    ...
    def line3d(self, fig: go.Figure, view: Literal['full', 'analysis'] = "full") -> None: 
        ...
        try: 
            ...
            raise ValueError(f"{view=} : this argument has to be set to 'full' or 'analysis'")

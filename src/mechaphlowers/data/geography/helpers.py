...
def gps_to_elevation(
    lat: np.ndarray[np.float64], lon: np.ndarray[np.float64]
) -> np.ndarray[np.float64]:
    """
    Fetch elevation data for a list of locations using Open-Elevation API

    Args:
        lat (np.ndarray[np.float64]): Latitude of the location in degrees
        lon (np.ndarray[np.float64]): Longitude of the location in degrees

    Returns:
        np.ndarray[np.float64]: Elevation in meters
    """
...
def reverse_haversine(
    ...
    bearing: np.ndarray[np.float64],
    distance: np.ndarray[np.float64],
    ...
    Args:
        lat (np.ndarray[np.float64]): Starting latitude in degrees
        lon (np.ndarray[np.float64]): Starting longitude in degrees
        bearing (np.ndarray[np.float64]): Bearing in degrees
        distance (np.ndarray[np.float64]): Distance in kilometers

    Returns:
        tuple[np.ndarray[np.float64], np.ndarray[np.float64]]: Tuple containing the latitude and longitude of the result
    """
...
def haversine(
    lat1: np.ndarray[np.float64],
    lon1: np.ndarray[np.float64],
    lat2: np.ndarray[np.float64],
    lon2: np.ndarray[np.float64],
) -> np.ndarray[np.float64]:
    """
    Calculate the great circle distance between two points
    on the earth (specified in decimal degrees)
    Args:
        lat1 (np.ndarray[np.float64]): Latitude of point A in degrees
        lon1 (np.ndarray[np.float64]): Longitude of point A in degrees
        lat2 (np.ndarray[np.float64]): Latitude of point B in degrees
        lon2 (np.ndarray[np.float64]): Longitude of point B in degrees

    Returns:
        np.ndarray[np.float64]: Distance in kilometers
    """
...
def gps_to_bearing(
    ...
    Returns:
        np.ndarray[np.float64]: Bearing angle in degrees from north (0-360)
    """
    lat1, lon1, lat2, lon2 = np.radians([lat1, lon1, lat2, lon2])
...
def bearing_to_direction(bearing: np.ndarray[np.float64]) -> str:
    """
    Convert bearing angle to cardinal direction name
    Args:
        bearing (np.ndarray[np.float64]): Bearing angle in degrees
...
def distances_to_gps(
    lat_a: np.ndarray[np.float64],
    lon_a: np.ndarray[np.float64],
    x_meters: np.ndarray[np.float64],
    y_meters: np.ndarray[np.float64],
) -> tuple[np.ndarray[np.float64], np.ndarray[np.float64]]:
    """
    Calculate GPS coordinates of point B given point A's coordinates and x,y distances in meters.

    Args:
        lat_a (np.ndarray[np.float64]): Latitude of point A in degrees
        lon_a (np.ndarray[np.float64]): Longitude of point A in degrees
        x_meters (np.ndarray[np.float64]): Distance from west to east in meters (positive = east, negative = west)
        y_meters (np.ndarray[np.float64]): Distance from south to north in meters (positive = north, negative = south)

    Returns:
        tuple[np.ndarray[np.float64], np.ndarray[np.float64]]: Tuple containing the latitude and longitude of point B
    """

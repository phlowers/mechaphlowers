...
class Frame:
    ...
    def rotation_axis(self) -> np.ndarray:
        ...
        # Do better than this
        axis_reshaped = np.reshape(np.ravel(self.axis, order="F"), (9, self.axis.shape[0]))
        ...
        rotation_axis_reshaped = np.reshape(np.ravel(rotation_axis, order="F"), (self.axis.shape[0],3))
        simple_truth_table: np.ndarray = np.all(rotation_axis_reshaped==0, axis=1)
        is_zero = np.repeat(simple_truth_table[:, np.newaxis], 3, axis=1)
        default_z_axis = np.array([[0,0,1]]).repeat(self.axis.shape[0], axis=0)
    ...
    def rotate_same_axis(self, line_angles_layer: np.ndarray, rotation_axis: np.ndarray):
        line_angle_array = np.repeat(line_angles_layer, self.axis.shape[1], axis=0)
...
class Points:
    ...
    def translate_all(self, translation_vector: np.ndarray):
        translation_vector_points = np.repeat(translation_vector[:, np.newaxis], self.coords.shape[1], axis=1)
    ...
    def rotate_all(self, line_angles_layer: np.ndarray, rotation_axes: np.ndarray):
    ...
    def transform(self, translation_vector: np.ndarray, line_angle: np.ndarray, rotation_axis: np.ndarray= np.array([0,0,1])):
        self.translate_all(translation_vector)
        self.rotate_all(line_angle, rotation_axis)
    
    def transform_frame(self, translation_vector: np.ndarray, line_angle: np.ndarray, rotation_axis: np.ndarray= np.array([0,0,1])):

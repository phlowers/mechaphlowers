...
def cable2span(x: np.ndarray, z: np.ndarray, beta: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray]: 
    ...
    elevation_part = np.linspace(tuple(np.nanmin(z,axis=1).tolist()), tuple(np.nanmax(z,axis=1).tolist()), x.shape[1]).T
    ...
    x_span,y_span,z_span = span[:,0].reshape(init_shape), span[:,1].reshape(init_shape), span[:,2].reshape(init_shape)
